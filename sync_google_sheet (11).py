...
Cac bang duoc dong bo trong ban nay: products, seasonality_index, pricing,
supply_chain_stock, inbound, monthly_sales, daily_orders, forecast,
ads_monthly, returns_monthly, freight_monthly, ads_listing, listing_sku_map.
...
def _get_values_retry(ws, retries=3, delay=5):
    """Doc get_all_values() co tu thu lai neu gap loi mang tam thoi
    (Connection reset, timeout...) - hay xay ra khi chay tren GitHub
    Actions, tranh phai vao bam Re-run tay moi lan."""
    last_exc = None
    for attempt in range(1, retries + 1):
        try:
            ...
            last_exc = exc
            print(f"[retry] Loi mang khi doc tab '{ws.title}' (lan {attempt}/{retries}): {exc}")
            if attempt < retries:
                time.sleep(delay)
    raise last_exc


def _worksheet_retry(sh, title, retries=3, delay=5):
    """Giong _get_values_retry nhung cho buoc MO tab (sh.worksheet(...)) -
    buoc nay cung goi API cua Google nen cung co the dinh loi mang/502
    tam thoi, truoc gio chua duoc retry nen 1 lan Google dot chung la
    fail ca job."""
    last_exc = None
    for attempt in range(1, retries + 1):
        try:
            ...
            last_exc = exc
            print(f"[retry] Loi mang khi mo tab '{title}' (lan {attempt}/{retries}): {exc}")
            if attempt < retries:
                time.sleep(delay)
    raise last_exc
...
def sync_products(sh):
    ws = _worksheet_retry(sh, "Product")
    rows = _get_values_retry(ws)

    # Cot "Index" (A/B/C/D) = nhom seasonality, dung de tra chi so o tab
    # Seasonality. Doc theo TEN cot truoc (giong Price/Ads), fallback ve
    # cot H (index 7) neu doi ten, tranh lap lai bug lech cot da gap truoc day.
    header = rows[0] if rows else []
    try:
        index_col = header.index("Index")
    except ValueError:
        index_col = 7
        print("[products] Khong tim thay cot 'Index' theo ten, dung fallback cot H (index 7).")
    ...
    for row in rows[1:]:
        ...
        records.append({
            ...
            "seasonality_group": cell(index_col).upper(),  # A/B/C/D
        })
    return records


# ============================================================
# seasonality_index <- tab "Seasonality"
# Bang: Month (1-12) | A | B | C | D -> long format (month, group, index)
# ============================================================
def sync_seasonality(sh):
    ws = _worksheet_retry(sh, "Seasonality")
    rows = _get_values_retry(ws)
    if not rows:
        return []
    header = rows[0]
    group_cols = [(i, header[i].strip()) for i in range(1, len(header)) if header[i].strip()]

    records = []
    for row in rows[1:]:
        if not row or _row_has_error(row):
            continue
        month_raw = row[0].strip() if row else ""
        if not month_raw:
            continue
        try:
            ...
            continue
        for col_idx, group in group_cols:
            if col_idx >= len(row):
                continue
            idx_val = _to_float(row[col_idx])
            if idx_val is None:
                continue
            records.append({
                "month": month,
                "product_group_letter": group,
                "index_value": idx_val,
            })
    return records
...
def sync_pricing(sh):
    ...
    idx = {
        "supplier_part_number": col_idx("Supplier Part Number"),
        "product_name": col_idx("Product Name"),
        "sku_internal": col_idx("SKU"),
        "base_cost": col_idx("Current Base Cost"),
        "cogs": col_idx("COGS"),
        "retail_price": col_idx("Retail Price"),
        "margin_pct": col_idx("Margin"),
        "breakeven_roas": col_idx("Break-even ROAS"),
    }

    if idx["supplier_part_number"] is None:
        print("[pricing] Khong tim thay cot 'Supplier Part Number', kiem tra lai ten cot trong sheet.")
        return []

    missing = [k for k, v in idx.items() if v is None and k != "breakeven_roas"]
    if missing:
        print(f"[pricing] Canh bao: khong tim thay cot cho {missing} - cac gia tri nay se la 0/rong.")
    ...
    for row in rows[1:]:
        if _row_has_error(row):
            continue
        part_no = cell(row, "supplier_part_number")
        if not part_no:
            continue
        records.append({
            "supplier_part_number": part_no,
            "sku_internal": cell(row, "sku_internal"),
            "cogs": _to_float(cell(row, "cogs")),
            "base_cost": _to_float(cell(row, "base_cost")),
            "retail_price": _to_float(cell(row, "retail_price")),
            "margin_pct": _to_float(cell(row, "margin_pct")),
        })
    return records
...
def sync_supply_chain_stock(sh):
    ws = _worksheet_retry(sh, "BC Tồn & ASIN")
    rows = _get_values_retry(ws)
...
# ============================================================
def sync_inbound(sh):
    ws = _worksheet_retry(sh, "INBOUND")
    rows = _get_values_retry(ws)
...
def sync_monthly_sales(sh):
    ws = _worksheet_retry(sh, "Monthly")
    rows = _get_values_retry(ws)
    period_row = rows[1]
    data_rows = rows[3:]
...
def sync_daily_orders(sh):
    ws = _worksheet_retry(sh, ORDERS_TAB_NAME)
    rows = _get_values_retry(ws)
...
def sync_forecast(sh):
    ws = _worksheet_retry(sh, "Forecast")
    rows = _get_values_retry(ws)
    header = rows[0]
    data_rows = rows[1:]
...
# ============================================================
def sync_freight(sh):
    ws = _worksheet_retry(sh, "Freight")
    rows = _get_values_retry(ws)
    records = []
    for row in rows[1:]:
        if _row_has_error(row):
...
# ============================================================
# freight_detail <- tab "Freight", 2 bang con nam canh bang tong theo thang:
#   cot G-L: Fulfillment fee + Unload fee theo tung Part Number
#   cot M-Q: Leaping fee (Processing + Storage) theo tung Part Number
# Gom theo (month, tp_code) de sau nay web tinh freight rieng cho tung
# product_group (join qua Part Number, giong cach da lam voi Return).
# ============================================================
def sync_freight_detail(sh):
    ws = _worksheet_retry(sh, "Freight")
    rows = _get_values_retry(ws)
    detail = {}  # (month, tp) -> {fulfillment_sum, unload_fee_sum, processing_fee, storage_fee}

    def add(month, tp, key, value):
        if not month or not tp:
            return
        k = (month, tp)
        if k not in detail:
            detail[k] = {"fulfillment_sum": 0.0, "unload_fee_sum": 0.0, "processing_fee": 0.0, "storage_fee": 0.0}
        detail[k][key] += value or 0.0

    for row in rows[1:]:
        if _row_has_error(row):
            continue

        # Bang 2: G(6)=Month H(7)=Part Number I(8)=fee/unit J(9)=Fulfillment Sum
        #         K(10)=Unload/unit L(11)=Unload Fee Sum
        if len(row) > 9:
            tp2_raw = row[7].strip() if len(row) > 7 else ""
            if tp2_raw and tp2_raw.upper() != "TOTAL":
                tps2 = _extract_tps(tp2_raw)
                month2 = _normalize_month(row[6].strip()) if len(row) > 6 else None
                if tps2 and month2:
                    add(month2, tps2[0], "fulfillment_sum", _to_float(row[9]) if len(row) > 9 else 0)
                    add(month2, tps2[0], "unload_fee_sum", _to_float(row[11]) if len(row) > 11 else 0)

        # Bang 3: M(12)=Month N(13)=Part Number O(14)=Total Order
        #         P(15)=Total Processing Fee Q(16)=Total Storage Fee
        if len(row) > 15:
            tp3_raw = row[13].strip() if len(row) > 13 else ""
            if tp3_raw and tp3_raw.upper() != "TOTAL":
                tps3 = _extract_tps(tp3_raw)
                month3 = _normalize_month(row[12].strip()) if len(row) > 12 else None
                if tps3 and month3:
                    add(month3, tps3[0], "processing_fee", _to_float(row[15]) if len(row) > 15 else 0)
                    add(month3, tps3[0], "storage_fee", _to_float(row[16]) if len(row) > 16 else 0)

    records = []
    for (month, tp), v in detail.items():
        total = v["fulfillment_sum"] + v["unload_fee_sum"] + v["processing_fee"] + v["storage_fee"]
        records.append({
            "month": month,
            "tp_code": tp,
            "fulfillment_sum": round(v["fulfillment_sum"], 2),
            "unload_fee_sum": round(v["unload_fee_sum"], 2),
            "processing_fee": round(v["processing_fee"], 2),
            "storage_fee": round(v["storage_fee"], 2),
            "total_freight": round(total, 2),
        })
    print(f"[freight_detail] {len(records)} dong (thang: {sorted(set(r['month'] for r in records))}).")
    return records


def sync_ads_monthly(sh):
    ws = _worksheet_retry(sh, "Ads")
    rows = _get_values_retry(ws)
    header = rows[0]
...
def _read_ads_tab(sh, tab_name):
    ...
    try:
        ws = _worksheet_retry(sh, tab_name)
    except Exception:
...
def build_ads_listing(sh):
    ...
    cogs_per_tp = {}
    try:
        price_rows = _worksheet_retry(sh, "Price").get_all_records(expected_headers=[
    ...
    try:
        ws = _worksheet_retry(sh, ORDERS_TAB_NAME)
        for row in _get_values_retry(ws)[1:]:
    ...
    tp_asin = {}
    try:
        prod_rows = _worksheet_retry(sh, "Product").get_all_values()
...
def main():
    ...
    upsert(supabase, "seasonality_index", sync_seasonality(sh), "product_group_letter", dedupe_keys="month,product_group_letter")
    ...
    upsert(supabase, "freight_detail", sync_freight_detail(sh), "month", dedupe_keys="month,tp_code")
