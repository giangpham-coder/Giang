...
def build_ads_listing(sh):
    """Tra ve (ads_listing_records, listing_sku_map_records)."""
    ...
    try:
        ...
        for row in ws.get_all_values()[1:]:
            if len(row) < 12 or _row_has_error(row):
                continue
            if "cancel" in row[8].strip().lower():      # I: Order Status
                continue
    ...
    if unmapped:
        ...
        print(f"[ads_listing] UNMAPPED ${total_unmapped:,.2f}: "
              + " ".join(f"{tp}(${u['rev']:.0f})" for (_m, tp), u in top))
...
def main():
    ...
    ads_listing_rows, sku_map_rows = build_ads_listing(sh)
